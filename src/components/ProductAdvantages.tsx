...
function Advantage({ title, description, icon }: AdvantageProps) {
  return (
    <div className="flex flex-col p-6 bg-background rounded-none border border-foreground/10 hover:border-primary/30 transition-colors">
      <div className="flex items-start gap-4 mb-4">
        <div className="w-10 h-10 flex-shrink-0 flex items-center justify-center bg-primary/10 rounded-none">
          <span className="text-primary" dangerouslySetInnerHTML={{ __html: icon }} />
        </div>
        <h3 className="text-xl font-heading" style={{ fontFamily: 'ClashDisplay-Semibold, sans-serif' }}>
          {title}
        </h3>
      </div>
      <p className="text-foreground/70 font-poppins ml-14">
        {description}
      </p>
    </div>
  );
  ...
  const advantages = [
    {
      ...
      description: "User intentions executed trustlessly through AI, without centralized control.",
    ...
    },
    {
      ...
      description: "Real-time predictive analysis for smarter, more profitable operations.",
      icon: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 3v11.25A2.25 2.25 0 0 0 6 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0 1 18 16.5h-2.25m-7.5 0h7.5m-7.5 0-1 3m8.5-3 1 3m0 0 .5 1.5m-.5-1.5h-9.5m0 0-.5 1.5m.75-9 3-3 2.148 2.148A12 12 0 0 1 21.75 8.25" /></svg>'
    },
    {
      title: "Open Ecosystem",
      description: "Developers contribute to a library of intent templates, monetizing expertise and expanding available strategies.",
    ...
    {
      title: "Seamless UX",
      description: "From simple natural language input to AI-executed blockchain actions.",
      icon: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" d="M7.5 8.25h9m-9 3H12m-9.75 1.51c0 1.6 1.123 2.994 2.707 3.227 1.129.166 2.27.293 3.423.379.35.026.67.21.865.501L12 21l2.755-4.133a1.14 1.14 0 0 1 .865-.501 48.172 48.172 0 0 0 3.423-.379c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0 0 12 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018Z" /></svg>'
    },
    {
      title: "Security by Design",
      description: "Intent encryption, permission controls, and on-chain transparency built in.",
      icon: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" /></svg>'
    }
  ];

  return (
    <section className="w-full py-24 border-t border-foreground/10" id="advantages">
      <Container>
